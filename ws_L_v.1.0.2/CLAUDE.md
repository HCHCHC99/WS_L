...
│   ├── cur_loop.c/h                 # 25kHz current PI loop (mode 10): per-EOCB 1:1, 5-tap sliding avg, Timer6 dt
...
STOP(0) → OPEN_FW(1)/OPEN_RV(2) → open-loop ramp at constant interval
       ...
       → CURLOOP_FW(10) → timed open loop (timer ramp anchored to calibrated table) → current PI (duty from ADC ISR)
```
...
- **4-channel DMA**: ADC1 CH0–CH3 via DMA1 CH0–CH3, 8-sample buffer per channel. BTC interrupt fires every 320µs (3.125kHz) at 25kHz PWM.
- **Trigger**: TMR4_3 SCMP0 at PWM counter peak (center-aligned triangle wave, 25kHz). EVT channel shares UH PWM channel — separate register sets, no known conflict.
...
> **Note**: PWM runs at 25kHz; the Biquad in `ws/I.c` is designed for fs=50kHz (`butter(2, 200/25000)`), so the real -3dB cutoff is ~100Hz (display only). The current loop uses a 5-tap sliding average of raw `g_i_*_ma`, not the Biquad output.
